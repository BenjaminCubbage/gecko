...
if not exist "%DYNAMIC_SECURITY_CONF_PATH%" (
    echo No dynamic security configuration found. Running mosquitto_ctrl dynsec init.
...
if exist "%MOSQUITTO_PID_PATH%" (
    for /f %%A in ('"%CMAKE_COMMAND%" -E cat %MOSQUITTO_PID_PATH%') do (
        echo Killing old mosquitto.exe instance with PID %%A. This required administrator priviledges.
        taskkill /F /PID %%A
    )
)

echo Running %MOSQUITTO_EXE_PATH% with configuration file %MOSQUITTO_CONF_PATH%
